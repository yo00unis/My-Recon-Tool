...
class Commands:
    ...
    def HttpxCommands():    
        commands = [
            f"httpx -l {GlobalEnv.GetSubDomainsPath()} -status-code -fc 404 -title -tech-detect -follow-redirects -json -o {GlobalEnv.GetTempJson()}",
        ]
        return commands
    ...
    def GowitnessCommands():
        commands = [
            f'gowitness scan file --file "{GlobalEnv.GetSubDomainsPath()}" --write-none --screenshot-path "{GlobalEnv.GetGowitness()}"'
        ]
        return commands
